...
@section('title', 'Project Packages')
@section('content')
    ...
    <div class="row align-items-end">
      <div class="col-lg-8">
        <div class="page-header-title">
          ...
          <div class="d-inline">
            <h5>{{ __('label.PROJECT_PACKAGES')}}</h5>
          </div>
        </div>
      </div>
      <div class="col-lg-4">
        <nav class="breadcrumb-container" aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">
              <a href="#">{{ __('label.PROJECT_PACKAGES')}}</a>
            </li>
          </ol>
        </nav>
      </div>
    ...
    <div class="col-md-12">
      <div class="card p-3">
        <div class="card-header">
          <h3>{{ __('label.PACKAGES_LIST')}}</h3>
          <div class="card-header-right">
              <a href="{{url('/project-packages/create')}}" class="btn btn-primary">  @lang('label.CREATE')</a>
          </div>
      </div>
        <div class="card-body">
          <table  class="table">
              ...
              <tr>
                ...
                <td scope="col">
                  <div class="table-actions">
                      <a href="{{route('project-packages.show', $value->id)}}" title="View">
                        <button class="mr-2 btn-icon btn-icon-only btn btn-primary">
                            <i class="fa fa-eye btn-icon-wrapper"></i>
                        </button>
                    </a>
                    <a href="{{route('project-packages.edit', $value->id)}}" title="Edit">
                        <button class="mr-2 btn-icon btn-icon-only btn btn-success">
                            <i class="fa fa-edit btn-icon-wrapper"></i>
                        </button>
                    </a>
                    <a>
                        <form action="{{url('project-packages/'.$value->id)}}" method="post">
                            @csrf
                            @method('DELETE')
